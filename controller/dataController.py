...
async def _init_table(user: str | None) -> bool:
    """_init_table: 初始化数据库结构"""

    if user != None:
        logger.warning("Admin %s is initizing table!", user)

    logger.info("Initizing dataset to file: %s", db_file_path)
    ...
    try:
        ...
        sql_template.close()
        del sql_template
        # logger.error("Error when initizing table: {0}".format(err))
        # return False
        raise err
    

async def _clean_all(user: str) -> bool:
    """
    _clean_all 的 Docstring
